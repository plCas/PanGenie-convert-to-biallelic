...
CompatibilityMode parse_compatibility_mode(const std::string& value) {
    if (value == "strict") {
        return CompatibilityMode::strict;
    }
    if (value == "python") {
        return CompatibilityMode::python;
    }
    throw std::invalid_argument("--compatibility must be strict or python");
}
...
Config parse_cli(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (option == "--compatibility") {
            mark_seen(seen, option);
            config.compatibility_mode = parse_compatibility_mode(
                require_value(argc, argv, index, "--compatibility"));
            continue;
        }
...
Optional options:
  ...
  --compatibility strict|python
                               strict keeps current behavior; python matches
                               convert-to-biallelic.py where deterministic.
